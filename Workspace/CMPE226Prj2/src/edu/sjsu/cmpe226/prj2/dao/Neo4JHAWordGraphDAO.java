...
public class Neo4JHAWordGraphDAO implements WordGraphDAO 
{
	...
	public Neo4JHAWordGraphDAO(String dbPath, String serverId, String serverCoordinates, String zkCoordinates, String remoteShell)
		throws Exception
	{
		_dbpath = dbPath;
		init(dbPath, serverId, serverCoordinates, zkCoordinates, remoteShell);
	}
	
	protected void init(String dbpath, String serverId, String serverCoordinates, String zkCoordinates, String remoteShell) 
		throws Exception
	{
		...
		System.out.println("Path :" + dbpath + ", serverId:" + serverId);
		
		Map<String, String> config = new HashMap<String, String>();
		config.put("ha.server_id", serverId);
		config.put("ha.server", serverCoordinates);
		config.put("ha.coordinators", zkCoordinates);
		config.put("enable_remote_shell", remoteShell);

		_db = new HighlyAvailableGraphDatabase( dbpath, config );		
		onShutdown(_db);
		...
		Runtime.getRuntime().addShutdownHook(new Thread() {
			...
			public void run() {
				try
				{
					...
					//System.err.println("Got Exception while shutdown !!" + ex);
					//ex.printStackTrace();
				}
			}
		});
